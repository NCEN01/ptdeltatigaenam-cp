        ...
        <div class="container">
                ...
                <div class="flex shrink-0 flex-col items-start gap-2.5 md:items-end" data-aos="fade-up">
                    <form action="{{ route('blog.index') }}" method="GET" role="search" class="relative w-full sm:w-72">
                        <input type="search" name="q" value="{{ $q ?? '' }}" autocomplete="off"
                               ...
                               class="w-full rounded-full border border-navy-200 bg-white py-2.5 pl-5 pr-12 text-sm text-navy placeholder:text-navy-300 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500/25">
                    ...
                    @if (($q ?? '') !== '')
                        <p class="font-mono text-xs text-navy-400">{{ $posts->total() }} {{ $id ? 'hasil untuk' : 'results for' }} “{{ $q }}” · <a href="{{ route('blog.index') }}" class="text-sky-600 hover:underline">{{ $id ? 'reset' : 'reset' }}</a></p>
                    @elseif ($posts->total())
                        <p class="font-mono text-xs text-navy-400">{{ str_pad($posts->total(), 2, '0', STR_PAD_LEFT) }} {{ $id ? 'artikel' : 'articles' }}</p>
                    @endif
                </div>
            </div>
            ...
            @else
                <div class="rounded-3xl border border-dashed border-navy-200 bg-neutral-50 p-16 text-center">
                    ...
                    <p class="mt-2 text-sm text-navy-500">{{ ($q ?? '') !== '' ? ($id ? 'Coba kata kunci lain atau lihat semua artikel.' : 'Try a different keyword or view all articles.') : ($id ? 'Artikel & wawasan terbaru akan tampil di sini.' : 'Our latest articles and insights will appear here.') }}</p>
                    @if (($q ?? '') !== '')
